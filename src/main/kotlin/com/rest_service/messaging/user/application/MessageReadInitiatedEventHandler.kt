...
@Singleton
@Named("MessageReadInitiatedEventHandler_userDomain")
class MessageReadInitiatedEventHandler(
    ...
    override fun mapDomainEvent(event: SagaEvent): DomainEvent {
        return userStateManager.mapDomainEvent(event.responsibleUserId!!, null, UserDomainEventType.MESSAGE_READ_APPROVED, event)
    }
    ...
    override fun handleError(event: SagaEvent, error: Throwable): Mono<Void> {
        return userStateManager.handleError(event, error, SagaEventType.MESSAGE_READ_REJECTED)
    }
}
