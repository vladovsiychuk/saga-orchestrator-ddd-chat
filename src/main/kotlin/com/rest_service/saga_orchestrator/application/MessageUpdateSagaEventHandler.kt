...
@Singleton
class MessageUpdateSagaEventHandler(
    ...
    override fun rebuildDomain(event: SagaEvent): Mono<Domain> {
        val messageSaga = MessageUpdateSaga(event.operationId, event.responsibleUserEmail, event.responsibleUserId!!)
    ...
    override fun handleError(event: SagaEvent, error: Throwable): Mono<Void> {
        return sagaStateManager.handleError(event, error, SagaEventType.MESSAGE_UPDATE_REJECTED)
    }
