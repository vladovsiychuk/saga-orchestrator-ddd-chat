...
@Singleton
class MessageCreateSagaEventHandler(
    ...
    override fun rebuildDomain(event: SagaEvent): Mono<Domain> {
        ...
        return repository.findByOperationIdOrderByDateCreated(operationId)
            .collectList()
            .flatMap { events ->
                val sagaState = MessageCreateSaga(operationId, event.responsibleUserEmail, event.responsibleUserId!!)
    ...
    override fun handleError(event: SagaEvent, error: Throwable): Mono<Void> {
        return repository.existsByOperationIdAndType(event.operationId, SagaEventType.MESSAGE_CREATE_REJECTED)
            .flatMap { exists ->
                if (exists)
